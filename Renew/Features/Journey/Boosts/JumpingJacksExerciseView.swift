...
struct JumpingJacksExerciseView: View {
    ...
    @State private var selectedDifficulty: JumpingJacksDifficulty = .starter
    @State private var isClaimingBoost = false
    @State private var showElectricGlow = false
    @State private var hasClaimedBoost = false
    
    enum JumpingJacksDifficulty: CaseIterable {
        case starter, energizer
        
        var title: String {
            switch self {
            case .starter: return "10 Jumping Jacks"
            case .energizer: return "25 Jumping Jacks"
            }
        }
        
        var description: String {
            switch self {
            case .starter: return "Quick pulse of energy"
            case .energizer: return "Amplify the rush"
            }
        }
    ...
    private var introSection: some View {
        VStack(alignment: .center, spacing: 14) {
            Text("⚡ Jumping Jacks Boost")
                .font(.largeTitle.weight(.bold))
                .foregroundColor(titleTextColor)
                .multilineTextAlignment(.center)
            
            Text("Spark your heart rate and mood in under a minute.")
                .font(.title3)
    ...
    private var difficultyOptionsSection: some View {
        VStack(spacing: 20) {
            ...
            ForEach(JumpingJacksDifficulty.allCases, id: \.self) { difficulty in
        ...
        ZStack {
            ...
            Button(action: claimBoost) {
                HStack(spacing: 12) {
                    ...
                    Text((isClaimingBoost || hasClaimedBoost) ? "POWERED UP!" : "Jacks Completed")
    ...
    private func difficultyCard(for difficulty: JumpingJacksDifficulty) -> some View {
        ...
        return HStack(spacing: 18) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    bulletRow("Start with feet together, arms relaxed.")
                    bulletRow("Jump feet wide while arms sweep overhead.")
                    bulletRow("Land softly on balls of feet, return arms to sides.")
                    bulletRow("Maintain a steady, upbeat rhythm.")
                }
            }
